...
{
    public partial class frm_fornecedor_excluidos : Form
    {
        Boolean add;
        ...
        public frm_fornecedor_excluidos(Boolean add)
        {
            InitializeComponent();
            this.add = add;
            //this.user = adm;
        ...
        private void frm_fornecedor_excluidos_Load(object sender, EventArgs e)
        {
        ...
        {
            dtg_for.DataSource = Servico.buscarFornecedorPorCategoria(cmb_categoria.SelectedIndex, textBox_pesquisa.Text, 1);
        ...
        {
            DialogResult res = MessageBox.Show("Deseja mesmo cadastrar um novo fornecedor DESATIVADO?", "Midas - Controle Financeiro", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (res == DialogResult.Yes)
            {
                //frmIndex frmp = new frmIndex(user);
                //Form_autenticaForne frm = new Form_autenticaForne();
                frm_fornecedor_excluidos frm = new frm_fornecedor_excluidos(true);
        ...
        private void toolStripButton_excluir_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult res = MessageBox.Show("Deseja mesmo ativar o " + textBox_razao.Text + "?", "Midas -  Controle Financeiro - Exclusão de Fornecedores", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (res == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        Servico.ativar(textBox_razao.Text);
                        MessageBox.Show("ativado com sucesso!", "Midas - Controle Financeiro - Ativação de Fornecedores", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.Hide();

                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("ERRO - " + ex, "Midas - Controle Financeiro - Ativação de Fornecedores", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
        ...
        private void toolStripButton_salvar_Click(object sender, EventArgs e)
        {
            if (pictureBox_email.Visible == true || pictureBox_site.Visible == true || pictureBox_email.Visible == true || pictureBox_cpf.Visible == true || pictureBox_fant.Visible == true)
            {
                if (!textBox_razao.Text.Length.Equals(0))
                {
                    DialogResult res = MessageBox.Show("Deseja mesmo cadastrar o " + textBox_razao.Text + " como seu fornecedor DESATIVADO?", "Midas -  Controle Financeiro - Cadastro de Fornecedores", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (res == DialogResult.Yes)
                    {
        ...
        {
            dtg_for.DataSource = Servico.buscarFornecedorPorCategoria(8, textBox_pesquisa.Text, 1);
        }
